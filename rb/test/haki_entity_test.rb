...
class HakiEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Haki(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Haki(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set ONE_PIECE_TEST_HAKI_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    haki_ref01_data_dt0_load_result = Helpers.to_map(haki_ref01_data_dt0_loaded.respond_to?(:data_get) ? haki_ref01_data_dt0_loaded.data_get : haki_ref01_data_dt0_loaded)
...
def haki_basic_setup(extra)
  ...
  entid_env_raw = ENV["ONE_PIECE_TEST_HAKI_ENTID"]
