...
class DialEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Dial(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Dial(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set ONE_PIECE_TEST_DIAL_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    dial_ref01_data_dt0_load_result = Helpers.to_map(dial_ref01_data_dt0_loaded.respond_to?(:data_get) ? dial_ref01_data_dt0_loaded.data_get : dial_ref01_data_dt0_loaded)
...
def dial_basic_setup(extra)
  ...
  entid_env_raw = ENV["ONE_PIECE_TEST_DIAL_ENTID"]
