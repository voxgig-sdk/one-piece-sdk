...
module OnePieceUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = OnePieceResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = OnePieceResponse.new(fetched)
    else
