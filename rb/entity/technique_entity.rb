...
class TechniqueEntity
  ...
  def make
    opts = @_entopts.dup
    TechniqueEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = OnePieceHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Technique, Hash] the current Technique data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = OnePieceHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Technique fields)
  def match_get
  ...
  #
  # @param reqmatch [TechniqueLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Technique.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Technique, Hash] the loaded Technique; raises OnePieceError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = OnePieceHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [TechniqueListMatch, Hash, nil] match filter (any subset of
  #   Technique fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Technique>, Array] the matching Technique items; raises OnePieceError on failure
