...
class ChapterEntity
  ...
  def make
    opts = @_entopts.dup
    ChapterEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.Chapter.load works with no args).
  ...
  #
  # @param reqmatch [ChapterListMatch, Hash, nil] match filter (any subset of
  #   Chapter fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
