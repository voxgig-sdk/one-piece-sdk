...
func TestCharacterEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Character(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Character(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set ONEPIECE_TEST_CHARACTER_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func characterBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("ONEPIECE_TEST_CHARACTER_ENTID")
	...
	idmapResolved := core.ToMapAny(env["ONEPIECE_TEST_CHARACTER_ENTID"])
