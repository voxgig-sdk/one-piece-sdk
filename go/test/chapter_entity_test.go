...
func TestChapterEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Chapter(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Chapter(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		chapterRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.chapter")))
		...
		chapterRef01DataDt0LoadResult := core.ToMapAny(entityData(chapterRef01DataDt0Loaded))
...
func chapterBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("ONE_PIECE_TEST_CHAPTER_ENTID")
	...
	idmapResolved := core.ToMapAny(env["ONE_PIECE_TEST_CHAPTER_ENTID"])
