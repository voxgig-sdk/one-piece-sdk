...
func TestLocationDirect(t *testing.T) {
	...
	t.Run("direct-load-location", func(t *testing.T) {
		setup := locationDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-location", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
func locationDirectSetup(mockres any) *locationDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"ONEPIECE_TEST_LOCATION_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["ONEPIECE_TEST_LOCATION_ENTID"]; ok {
