...
func TestCrewEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set ONE_PIECE_TEST_CREW_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		crewRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.crew")))
...
func crewBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("ONE_PIECE_TEST_CREW_ENTID")
	...
	idmapResolved := core.ToMapAny(env["ONE_PIECE_TEST_CREW_ENTID"])
