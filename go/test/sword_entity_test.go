...
func TestSwordEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Sword(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Sword(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		swordRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.sword")))
		...
		swordRef01DataDt0LoadResult := core.ToMapAny(entityData(swordRef01DataDt0Loaded))
...
func swordBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("ONE_PIECE_TEST_SWORD_ENTID")
	...
	idmapResolved := core.ToMapAny(env["ONE_PIECE_TEST_SWORD_ENTID"])
