...
//
// NewOnePieceSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.OnePieceSDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
	...
	return sdk.NewOnePieceSDK(opts)
}
...
func fcCandidates(client *sdk.OnePieceSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.OnePieceSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.OnePieceSDK, name string) any {
