...
import (
	...
	omni "github.com/voxgig-sdk/one-piece-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.OnePieceError); is {
				out := map[string]any{
					"name":    "OnePieceError",
...
func omniCtx(arg any, client *sdk.OnePieceSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.OnePieceSDK); is {
			client = live
