// Command OnePiece-cli is a CLI + REPL for the OnePiece SDK,
...
const prompt = "one-piece"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "boat bow chapter character crew dial episode film fruit gear haki location saga sword technique volume"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      ONE_PIECE_APIKEY (api key), ONE_PIECE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: ONE_PIECE_APIKEY carries the API key and
	// ONE_PIECE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("ONE_PIECE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("ONE_PIECE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewOnePieceSDK(opts)
