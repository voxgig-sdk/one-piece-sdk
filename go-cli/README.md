...
boru-driven command-line client **and** interactive REPL for the OnePiece
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/one-piece-cli)
make build
...
./one-piece-cli list boat
./one-piece-cli load 1 boat            # {id:1} shorthand
./one-piece-cli load '{id:1}' boat       # explicit match map
./one-piece-cli list bow

# 5. Override the API base URL for a single call
ONE_PIECE_BASE=https://api.example.com ./one-piece-cli list boat
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/one-piece-cli
   ```
...
```sh
./one-piece-cli load 1 boat          # scalar shorthand for {id:1}
./one-piece-cli load '{id:1}' boat     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `one-piece>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `boat`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/one-piece-cli`. |
...
(`./one-piece-cli <expr>`) and interactively (the REPL), and expressions compose the
