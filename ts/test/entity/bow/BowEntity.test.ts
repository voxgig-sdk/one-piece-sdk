...
describe('BowEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'bow.' + op, live)) return
    }
    ...
    const bow_ref01_list = await bow_ref01_ent.list(bow_ref01_match)


    // LOAD
    const bow_ref01_match_dt0: any = {}
    bow_ref01_match_dt0.id = bow_ref01_data.id
    const bow_ref01_data_dt0 = await bow_ref01_ent.load(bow_ref01_match_dt0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['ONE_PIECE_TEST_BOW_ENTID']
