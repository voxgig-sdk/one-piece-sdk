...
(0, node_test_1.describe)('LocationDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-location', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-location', setup.live))
            return;
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['ONE_PIECE_TEST_LOCATION_ENTID'];
