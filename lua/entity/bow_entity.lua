...
local BowEntity = {}
BowEntity.__index = BowEntity


function BowEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BowEntity)
  self._name = "bow"
  self._client = client
...
function BowEntity:get_name()
  return self._name
end


function BowEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BowEntity.new(self._client, opts)
end
...
function BowEntity:mark_deleted()
  self._deleted = true
end


function BowEntity:deleted()
  return true == self._deleted
end


function BowEntity:data_set(args)
  if args ~= nil then
...
function BowEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BowEntity:match_set(args)
  if args ~= nil then
...
function BowEntity:stream(action, args, callopts)
...
---@param reqmatch BowLoadMatch
---@param ctrl? table
---@return Bow
---@return string? err
function BowEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch BowListMatch
---@param ctrl? table
---@return Bow[]
---@return string? err
function BowEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function BowEntity:_run_op(ctx, post_done)
  local utility = self._utility
