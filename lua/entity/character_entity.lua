...
function CharacterEntity.new(client, entopts)
  entopts = entopts or {}
...
function CharacterEntity:get_name()
  return self._name
end


function CharacterEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CharacterEntity.new(self._client, opts)
end
...
function CharacterEntity:mark_deleted()
  self._deleted = true
end


function CharacterEntity:deleted()
  return true == self._deleted
end


function CharacterEntity:data_set(args)
  if args ~= nil then
...
function CharacterEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CharacterEntity:match_set(args)
  if args ~= nil then
...
function CharacterEntity:match_get()
...
function CharacterEntity:stream(action, args, callopts)
...
function CharacterEntity:_run_op(ctx, post_done)
  local utility = self._utility
