...
local FilmEntity = {}
FilmEntity.__index = FilmEntity


function FilmEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FilmEntity)
  self._name = "film"
  self._client = client
...
function FilmEntity:get_name()
  return self._name
end


function FilmEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FilmEntity.new(self._client, opts)
end
...
function FilmEntity:mark_deleted()
  self._deleted = true
end


function FilmEntity:deleted()
  return true == self._deleted
end


function FilmEntity:data_set(args)
  if args ~= nil then
...
function FilmEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FilmEntity:match_set(args)
  if args ~= nil then
...
function FilmEntity:stream(action, args, callopts)
...
---@param reqmatch FilmLoadMatch
---@param ctrl? table
---@return Film
---@return string? err
function FilmEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch FilmListMatch
---@param ctrl? table
---@return Film[]
---@return string? err
function FilmEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function FilmEntity:_run_op(ctx, post_done)
  local utility = self._utility
