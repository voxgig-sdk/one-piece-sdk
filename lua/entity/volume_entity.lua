...
local VolumeEntity = {}
VolumeEntity.__index = VolumeEntity


function VolumeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, VolumeEntity)
  self._name = "volume"
  self._client = client
...
function VolumeEntity:get_name()
  return self._name
end


function VolumeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return VolumeEntity.new(self._client, opts)
end
...
function VolumeEntity:mark_deleted()
  self._deleted = true
end


function VolumeEntity:deleted()
  return true == self._deleted
end


function VolumeEntity:data_set(args)
  if args ~= nil then
...
function VolumeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function VolumeEntity:match_set(args)
  if args ~= nil then
...
function VolumeEntity:match_get()
...
function VolumeEntity:stream(action, args, callopts)
...
---@param reqmatch VolumeLoadMatch
---@param ctrl? table
---@return Volume
---@return string? err
function VolumeEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch VolumeListMatch
---@param ctrl? table
---@return Volume[]
---@return string? err
function VolumeEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function VolumeEntity:_run_op(ctx, post_done)
  local utility = self._utility
