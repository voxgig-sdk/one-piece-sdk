...
local HakiEntity = {}
HakiEntity.__index = HakiEntity


function HakiEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, HakiEntity)
  self._name = "haki"
  self._client = client
...
function HakiEntity:get_name()
  return self._name
end


function HakiEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return HakiEntity.new(self._client, opts)
end
...
function HakiEntity:mark_deleted()
  self._deleted = true
end


function HakiEntity:deleted()
  return true == self._deleted
end


function HakiEntity:data_set(args)
  if args ~= nil then
...
function HakiEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function HakiEntity:match_set(args)
  if args ~= nil then
...
function HakiEntity:stream(action, args, callopts)
...
---@param reqmatch HakiLoadMatch
---@param ctrl? table
---@return Haki
---@return string? err
function HakiEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch HakiListMatch
---@param ctrl? table
---@return Haki[]
---@return string? err
function HakiEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function HakiEntity:_run_op(ctx, post_done)
  local utility = self._utility
