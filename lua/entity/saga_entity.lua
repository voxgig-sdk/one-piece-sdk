...
local SagaEntity = {}
SagaEntity.__index = SagaEntity


function SagaEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SagaEntity)
  self._name = "saga"
  self._client = client
...
function SagaEntity:get_name()
  return self._name
end


function SagaEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SagaEntity.new(self._client, opts)
end
...
function SagaEntity:mark_deleted()
  self._deleted = true
end


function SagaEntity:deleted()
  return true == self._deleted
end


function SagaEntity:data_set(args)
  if args ~= nil then
...
function SagaEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SagaEntity:match_set(args)
  if args ~= nil then
...
function SagaEntity:stream(action, args, callopts)
...
function SagaEntity:_run_op(ctx, post_done)
  local utility = self._utility
