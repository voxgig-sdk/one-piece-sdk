-- OnePiece SDK Fruit entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local FruitEntity = {}
FruitEntity.__index = FruitEntity


function FruitEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FruitEntity)
  self._name = "fruit"
  self._client = client
...
function FruitEntity:get_name()
  return self._name
end


function FruitEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FruitEntity.new(self._client, opts)
end
...
function FruitEntity:mark_deleted()
  self._deleted = true
end


function FruitEntity:deleted()
  return true == self._deleted
end


function FruitEntity:data_set(args)
  if args ~= nil then
...
function FruitEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FruitEntity:match_set(args)
  if args ~= nil then
...
function FruitEntity:match_get()
...
function FruitEntity:stream(action, args, callopts)
...
function FruitEntity:_run_op(ctx, post_done)
  local utility = self._utility
