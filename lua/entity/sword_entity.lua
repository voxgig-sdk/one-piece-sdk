-- OnePiece SDK Sword entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local SwordEntity = {}
SwordEntity.__index = SwordEntity


function SwordEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SwordEntity)
  self._name = "sword"
  self._client = client
...
function SwordEntity:get_name()
  return self._name
end


function SwordEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SwordEntity.new(self._client, opts)
end
...
function SwordEntity:mark_deleted()
  self._deleted = true
end


function SwordEntity:deleted()
  return true == self._deleted
end


function SwordEntity:data_set(args)
  if args ~= nil then
...
function SwordEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SwordEntity:match_set(args)
  if args ~= nil then
...
function SwordEntity:match_get()
...
function SwordEntity:stream(action, args, callopts)
...
---@param reqmatch SwordLoadMatch
---@param ctrl? table
---@return Sword
---@return string? err
function SwordEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch SwordListMatch
---@param ctrl? table
---@return Sword[]
---@return string? err
function SwordEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function SwordEntity:_run_op(ctx, post_done)
  local utility = self._utility
