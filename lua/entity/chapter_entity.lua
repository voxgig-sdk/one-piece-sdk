...
function ChapterEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ChapterEntity)
  self._name = "chapter"
  self._client = client
...
function ChapterEntity:get_name()
  return self._name
end


function ChapterEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ChapterEntity.new(self._client, opts)
end
...
function ChapterEntity:mark_deleted()
  self._deleted = true
end


function ChapterEntity:deleted()
  return true == self._deleted
end


function ChapterEntity:data_set(args)
  if args ~= nil then
...
function ChapterEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ChapterEntity:match_set(args)
  if args ~= nil then
...
function ChapterEntity:match_get()
...
function ChapterEntity:stream(action, args, callopts)
...
---@param reqmatch ChapterLoadMatch
---@param ctrl? table
---@return Chapter
---@return string? err
function ChapterEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch ChapterListMatch
---@param ctrl? table
---@return Chapter[]
---@return string? err
function ChapterEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function ChapterEntity:_run_op(ctx, post_done)
  local utility = self._utility
