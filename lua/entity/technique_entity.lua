...
local TechniqueEntity = {}
TechniqueEntity.__index = TechniqueEntity


function TechniqueEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TechniqueEntity)
  self._name = "technique"
  self._client = client
...
function TechniqueEntity:get_name()
  return self._name
end


function TechniqueEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TechniqueEntity.new(self._client, opts)
end
...
function TechniqueEntity:mark_deleted()
  self._deleted = true
end


function TechniqueEntity:deleted()
  return true == self._deleted
end


function TechniqueEntity:data_set(args)
  if args ~= nil then
...
function TechniqueEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TechniqueEntity:match_set(args)
  if args ~= nil then
...
function TechniqueEntity:match_get()
...
function TechniqueEntity:stream(action, args, callopts)
...
function TechniqueEntity:_run_op(ctx, post_done)
  local utility = self._utility
