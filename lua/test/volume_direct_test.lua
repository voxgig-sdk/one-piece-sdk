...
describe("VolumeDirect", function()
  it("should direct-list-volume", function()
    local setup = volume_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-volume", setup.live and "live" or "unit")
  ...
  it("should direct-load-volume", function()
    local setup = volume_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-volume", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function volume_direct_setup(mockres)
