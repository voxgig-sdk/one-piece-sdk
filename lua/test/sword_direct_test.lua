...
describe("SwordDirect", function()
  it("should direct-list-sword", function()
    local setup = sword_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-sword", setup.live and "live" or "unit")
  ...
  it("should direct-load-sword", function()
    local setup = sword_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-sword", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function sword_direct_setup(mockres)
