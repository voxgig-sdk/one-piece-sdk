...
describe("CharacterDirect", function()
  it("should direct-list-character", function()
    local setup = character_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-character", setup.live and "live" or "unit")
  ...
  it("should direct-load-character", function()
    local setup = character_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-character", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function character_direct_setup(mockres)
