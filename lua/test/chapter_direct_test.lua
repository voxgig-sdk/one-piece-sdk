...
describe("ChapterDirect", function()
  it("should direct-list-chapter", function()
    local setup = chapter_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-chapter", setup.live and "live" or "unit")
  ...
  it("should direct-load-chapter", function()
    local setup = chapter_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-chapter", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function chapter_direct_setup(mockres)
