...
describe("BowDirect", function()
  it("should direct-list-bow", function()
    local setup = bow_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-bow", setup.live and "live" or "unit")
  ...
  it("should direct-load-bow", function()
    local setup = bow_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-bow", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function bow_direct_setup(mockres)
