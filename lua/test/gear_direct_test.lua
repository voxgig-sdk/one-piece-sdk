...
describe("GearDirect", function()
  it("should direct-list-gear", function()
    local setup = gear_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-gear", setup.live and "live" or "unit")
  ...
  it("should direct-load-gear", function()
    local setup = gear_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-gear", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function gear_direct_setup(mockres)
