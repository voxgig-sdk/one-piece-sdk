...
describe("SagaDirect", function()
  it("should direct-list-saga", function()
    local setup = saga_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-saga", setup.live and "live" or "unit")
  ...
  it("should direct-load-saga", function()
    local setup = saga_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-saga", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function saga_direct_setup(mockres)
