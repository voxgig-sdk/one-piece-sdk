...
describe("FilmDirect", function()
  it("should direct-list-film", function()
    local setup = film_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-film", setup.live and "live" or "unit")
  ...
  it("should direct-load-film", function()
    local setup = film_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-film", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function film_direct_setup(mockres)
