...
describe("HakiDirect", function()
  it("should direct-list-haki", function()
    local setup = haki_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-haki", setup.live and "live" or "unit")
  ...
  it("should direct-load-haki", function()
    local setup = haki_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-haki", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function haki_direct_setup(mockres)
