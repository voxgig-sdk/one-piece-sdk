...
describe("FruitEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Fruit(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Fruit(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set ONE_PIECE_TEST_FRUIT_ENTID JSON to run live")
      return
    end
...
function fruit_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("ONE_PIECE_TEST_FRUIT_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["ONE_PIECE_TEST_FRUIT_ENTID"])
