...
describe("TechniqueDirect", function()
  it("should direct-list-technique", function()
    local setup = technique_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-technique", setup.live and "live" or "unit")
  ...
  it("should direct-load-technique", function()
    local setup = technique_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-technique", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function technique_direct_setup(mockres)
