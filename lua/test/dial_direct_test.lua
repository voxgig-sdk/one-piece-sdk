...
describe("DialDirect", function()
  it("should direct-list-dial", function()
    local setup = dial_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-dial", setup.live and "live" or "unit")
  ...
  it("should direct-load-dial", function()
    local setup = dial_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-dial", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function dial_direct_setup(mockres)
