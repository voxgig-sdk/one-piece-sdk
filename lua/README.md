...
The Lua SDK for the OnePiece API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Boat(nil):list(nil, nil)
...
```lua
local result, err = client:Boat(nil):load({ id = "example_id" }, nil)
if err then error(err) end
print(result)
```
...
local result, err = client:OnePiece(nil):load(
  { id = "test01" }, nil
)
