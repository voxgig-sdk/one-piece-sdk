...
class FtCtrl extends OnePieceControl
{
...
class FtClient
{
    ...
    private OnePieceUtility $utility;

    public function __construct(OnePieceUtility $utility, array $options)
    {
    ...
    public function get_utility(): OnePieceUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (OnePieceContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public OnePieceContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (OnePieceContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = OnePieceConfig::make_feature($name);
    ...
    private static function buildUrl(OnePieceSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof OnePieceError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new OnePieceResponse($fetched);
        }
    ...
    private function fail(OnePieceContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (OnePieceContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
