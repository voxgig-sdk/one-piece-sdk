...
class OnePieceNetsimFeature extends OnePieceBaseFeature
{
    ...
    private function _simulate(OnePieceContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(OnePieceContext $ctx, array $applied): void
    {
        $client = $this->client;
