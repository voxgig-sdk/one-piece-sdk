...
class TechniqueEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new TechniqueEntity($this->_client, $opts);
    }

    /**
     * @param Technique|array $args Technique data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = OnePieceHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Technique|array The current Technique data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Technique fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = OnePieceHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Technique fields).
     */
    ...
    /**
     * Load a single Technique.
     *
     * @param TechniqueLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed TechniqueLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Technique|array The loaded Technique as an assoc-array at the
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = OnePieceHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
    ...
    /**
     * List Technique items matching the given filter.
     *
     * @param TechniqueListMatch|array|null $reqmatch Match filter (any subset
     *   of Technique fields) as an assoc-array; TechniqueListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Technique[]|array A list of Technique items as assoc-arrays at
