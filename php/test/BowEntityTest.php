...
class BowEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Bow(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Bow(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set ONE_PIECE_TEST_BOW_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $bow_ref01_data_dt0_load_result = Helpers::to_map(is_object($bow_ref01_data_dt0_loaded) && method_exists($bow_ref01_data_dt0_loaded, 'data_get') ? $bow_ref01_data_dt0_loaded->data_get() : $bow_ref01_data_dt0_loaded);
...
function bow_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("ONE_PIECE_TEST_BOW_ENTID");
