...
class FilmEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = OnePieceSDK::test(null, null);
        $ent = $testsdk->Film(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Film(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Film(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = film_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "film." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set ONE_PIECE_TEST_FILM_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $film_ref01_data = null;
        if (count($film_ref01_data_raw) > 0) {
...
function film_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/film/FilmTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["film01", "film02", "film03"] as $k) {
    ...
    $entid_env_raw = getenv("ONE_PIECE_TEST_FILM_ENTID");
