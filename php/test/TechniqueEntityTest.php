...
class TechniqueEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set ONE_PIECE_TEST_TECHNIQUE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $technique_ref01_data_dt0_load_result = Helpers::to_map(is_object($technique_ref01_data_dt0_loaded) && method_exists($technique_ref01_data_dt0_loaded, 'data_get') ? $technique_ref01_data_dt0_loaded->data_get() : $technique_ref01_data_dt0_loaded);
...
function technique_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("ONE_PIECE_TEST_TECHNIQUE_ENTID");
