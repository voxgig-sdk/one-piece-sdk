...
class DialEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = OnePieceSDK::test(null, null);
        $ent = $testsdk->Dial(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Dial(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Dial(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = dial_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "dial." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set ONE_PIECE_TEST_DIAL_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $dial_ref01_data = null;
        if (count($dial_ref01_data_raw) > 0) {
        ...
        $dial_ref01_data_dt0_load_result = Helpers::to_map(is_object($dial_ref01_data_dt0_loaded) && method_exists($dial_ref01_data_dt0_loaded, 'data_get') ? $dial_ref01_data_dt0_loaded->data_get() : $dial_ref01_data_dt0_loaded);
...
function dial_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/dial/DialTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["dial01", "dial02", "dial03"] as $k) {
    ...
    $entid_env_raw = getenv("ONE_PIECE_TEST_DIAL_ENTID");
