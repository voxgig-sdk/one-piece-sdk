...
class HakiEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = OnePieceSDK::test(null, null);
        $ent = $testsdk->Haki(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Haki(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Haki(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = haki_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "haki." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set ONE_PIECE_TEST_HAKI_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $haki_ref01_data = null;
        if (count($haki_ref01_data_raw) > 0) {
        ...
        $haki_ref01_data_dt0_load_result = Helpers::to_map(is_object($haki_ref01_data_dt0_loaded) && method_exists($haki_ref01_data_dt0_loaded, 'data_get') ? $haki_ref01_data_dt0_loaded->data_get() : $haki_ref01_data_dt0_loaded);
...
function haki_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/haki/HakiTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["haki01", "haki02", "haki03"] as $k) {
    ...
    $entid_env_raw = getenv("ONE_PIECE_TEST_HAKI_ENTID");
