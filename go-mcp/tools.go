...
type Args struct {
	Entity string         `json:"entity" jsonschema:"boat | bow | chapter | character | crew | dial | episode | film | fruit | gear | haki | location | saga | sword | technique | volume"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.OnePieceSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "one-piece_list",
		Description: "List records from OnePiece. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "one-piece_load",
		Description: "Load a single record from OnePiece. " +
...
func runOp(client *sdk.OnePieceSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.OnePieceSDK, name string) (sdk.OnePieceEntity, error) {
	switch strings.ToLower(name) {
	case "boat":
		return client.Boat(nil), nil
	case "bow":
		return client.Bow(nil), nil
	case "chapter":
		return client.Chapter(nil), nil
	case "character":
		return client.Character(nil), nil
	case "crew":
		return client.Crew(nil), nil
	case "dial":
		return client.Dial(nil), nil
	case "episode":
		return client.Episode(nil), nil
	case "film":
		return client.Film(nil), nil
	case "fruit":
		return client.Fruit(nil), nil
	case "gear":
		return client.Gear(nil), nil
	case "haki":
		return client.Haki(nil), nil
	case "location":
		return client.Location(nil), nil
	case "saga":
		return client.Saga(nil), nil
	case "sword":
		return client.Sword(nil), nil
	case "technique":
		return client.Technique(nil), nil
	case "volume":
		return client.Volume(nil), nil
