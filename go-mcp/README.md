...
[MCP](https://modelcontextprotocol.io) server exposing the OnePiece SDK as
two agent tools — `one-piece_list` and `one-piece_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/one-piece-mcp)
make build

# 2. Provide credentials via the environment
export ONE_PIECE_APIKEY=sk_live_xxx
...
```jsonc
// one-piece_list: first page of records
{ "entity": "boat" }
{ "entity": "boat", "query": { } }

// one-piece_load: one record by id
{ "entity": "boat", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/one-piece-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user one-piece \
     -- "$PWD"/dist/*/one-piece-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `one-piece_list` and `one-piece_load` tools now appear
   in new sessions. Ask the agent to *"list boat using one-piece"*
   and it calls `one-piece_list` with `{"entity":"boat"}`.
...
```sh
export ONE_PIECE_APIKEY=sk_live_xxx            # API key
export ONE_PIECE_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `one-piece_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `one-piece_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `ONE_PIECE_APIKEY` | API key sent with every request. |
| `ONE_PIECE_BASE` | Optional override of the API base URL. |
